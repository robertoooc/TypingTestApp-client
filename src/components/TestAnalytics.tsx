...
import { mistakes, userData, test } from "./types/types";
...
const TestAnalytics: FC<Props> = ({ userData }) => {
  ...
  const handleCompare = () => {
    let wpmPercentage,
      wpmDifference,
      mistakeAmountPercentage,
      mistakeAmountDifference,
      accuracyDifference,
      accuracyPercentage;
    if (test1Wpm == test2Wpm) {
      ...
      test2Wpm == 0
        ? (wpmPercentage = parseFloat(
            (((test1Wpm - 0) / Math.abs(1)) * 100).toFixed(2)
          ))
        : (wpmPercentage = parseFloat(
            (((test1Wpm - test2Wpm) / Math.abs(test2Wpm)) * 100).toFixed(2)
          ));
    ...
    if (test1MistakeAmount == test2MistakeAmount) {
      ...
      test2MistakeAmount == 0
        ? (mistakeAmountPercentage = parseFloat(
            (((test1MistakeAmount - 0) / Math.abs(1)) * 100).toFixed(2)
          ))
        : (mistakeAmountPercentage = parseFloat(
            (
              ((test1MistakeAmount - test2MistakeAmount) /
                Math.abs(test2MistakeAmount)) *
              100
            ).toFixed(2)
          ));
    ...
    if (test1Accuracy == test2Accuracy) {
      ...
      test2Accuracy == 0
        ? (accuracyPercentage = parseFloat(
            (((test1Accuracy - 0) / Math.abs(1)) * 100).toFixed(2)
          ))
        : (accuracyPercentage = parseFloat(
            (
              ((test1Accuracy - test2Accuracy) / Math.abs(test2Accuracy)) *
              100
            ).toFixed(2)
          ));
  ...
  if (userData?.length == 0) {
    ...
    viewData = userData?.map((test) => {
      ...
      test?.mistakes?.length > 0
        ? (mistakeAmount = test.mistakes
            .map((item) => item.amount)
            .reduce((prev, next) => prev + next))
        : (mistakeAmount = 0);
      date = test.time.slice(0, 10);
      date = `${date.substring(5, 7)}/${date.substring(8, 10)}/${date.substring(
        0,
        4
      )}`;
      ...
      return (
        <div
          key={`${test._id}`}
          className="flex place-content-around items-center my-1 bg-stone-200 rounded-lg"
          onClick={() =>
            handleTest1(test.wpm, mistakeAmount, test.accuracy, test)
          }
        >
          <div className="">
    ...
    viewData2 = userData?.map((test) => {
      ...
      return (
        <div
          ...
          className="flex place-content-around items-center my-1 bg-stone-200 rounded-lg"
          onClick={() =>
            handleTest2(test.wpm, mistakeAmount, test.accuracy, test)
          }
        >
          <div className="">
            <p className="font-sans text-base">{test.wpm}</p>
    ...
    <div>
      ...
      <div className="flex place-content-evenly">
        <div>
          ...
          <div>
            {wpmDifference < 0 ? (
              <p className="text-center">{wpmDifference * -1} less WPM</p>
            ) : (
              <p className="text-center">{wpmDifference} more WPM</p>
            )}
          </div>
          <div>
            {wpmPercentage < 0 ? (
              <p className="text-center">{wpmPercentage * -1}% decline</p>
            ) : (
              <p className="text-center">{wpmPercentage}% increase</p>
            )}
          </div>
        </div>
        <div>
          <p className="text-center">
            Test-1 compared to Test-2: Spelling Accuray
          </p>
          <div>
            {accuracyPercentage < 0 ? (
              <p className="text-center">
                {accuracyPercentage * -1}% decreased spelling accuracy
              </p>
            ) : (
              <p className="text-center">
                {accuracyPercentage}% increased spelling accuracy
              </p>
            )}
          </div>
        </div>
        <div>
          <p className="text-center">
            Test-1 compared to Test-2: Total Mistakes
          </p>
          <div>
            {mistakeDifference < 0 ? (
              <p className="text-center">
                {mistakeDifference * -1} less mistakes
              </p>
            ) : (
              <p className="text-center">{mistakeDifference} more mistakes</p>
            )}
          </div>
          <div>
            {mistakePercentage < 0 ? (
              <p className="text-center">{mistakePercentage * -1}% decrease</p>
            ) : (
              <p className="text-center">{mistakePercentage}% increase</p>
            )}
          </div>
        </div>
      </div>
