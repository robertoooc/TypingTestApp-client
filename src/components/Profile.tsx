...
import { currentUser, mistakes, userData, accountInfo } from "./types/types";
interface Props {
...
const Profile: FC<Props> = ({ currentUser }) => {
  ...
  const deleteAccount = async () => {
    try {
      ...
      const changePassword = await axios.delete(
  ...
  const edit = (
    <div>
      <form
        ...
        className="max-w-[250px] w-full m-10 bg-zinc-800 p-8 px-8 rounded-lg"
      >
        ...
        <div className="flex items-center w-full my-2 py-2 bg-zinc-700 text-white font-semibold rounded-lg">
          <span onClick={deleteAccount} className="pl-5">
            Delete Account{" "}
          </span>
  ...
  return (
    <div className="mx-auto">
      ...
      <button
        ...
        className=" bg-zinc-800 rounded-lg text-white p-2 font-medium m-2 "
      >
        Analytics
      </button>
